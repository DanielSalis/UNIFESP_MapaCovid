...
const FilterBar = (props) => {
  ...
  useEffect(() => {
    async function fetchDataStates() {
      if (!allStates) {
        await api.get("/api/map/get/states").then(async (res) => {
  ...
  const fetchDataCases = async (obj) => {
    const params = {
      id: obj.city == null ? obj.state.codigo_uf : obj.city.codigo_ibge,
    };
    const endpoint = obj.city == null ? "casesByState" : "casesByCity";

    api
      .get(`/api/map/get/${endpoint}`, {
        params: params,
      })
      .then(async (res) => {
  ...
  return (
    <Container>
      <UpperDiv>
        <UpperDivLeft>
          {props.map.state ? (
            <SelectedInputs local="states" text={props.map.state} />
          ) : (
            <></>
          )}
          {props.map.city ? (
      ...
      <BottomDiv>
        ...
        <input
          onChange={(e) => handleDatalistChange(e, "states")}
        ...
        <datalist
          ...
          id="exampleList"
        >
          {allStates && country === false ? (
            allStates.map((item) => {
              return <option key={item.codigo_uf} value={item.nome} />;
            })
          ) : (
            <option value={""} />
          )}
        ...
        <input
          onChange={(e) => handleDatalistChange(e, "cities")}
          disabled={
            country === true ||
            props.map.state === null ||
            props.map.state === ""
          }
          type="text"
          name="Cities"
        ...
        <datalist id="dataListCidades">
          {citiesByState && country === false
            ? citiesByState.map((item, index) => {
                return (
                  <option
                    key={index}
                    codigo={item.codigo_ibge}
                    value={item.nome}
                  />
                );
              })
